...
parser.add_argument('-o', '--output', help = 'output file', nargs = '?', default = None, const = '1stNUP.xml')
...
def image_paths(v):
    ...
    v2 = hex_to_dec(v1)
    v3 = dec_to_hex(v2)
    return [v1, f'{img_path}{v1}.tga', f'{img_path}{v1}.png', f'{img_path}{v2}.tga', f'{img_path}{v2}.png', f'{img_path}{v3}.tga', f'{img_path}{v3}.png']
...
def encode_image(value, size):
    if len(value) > max_path_len: return value
...
def default_elements(target, source):
    for (key, val) in source.items(): ET.SubElement(target, key).text = val
    return target

def optional_text(element, sub, text):
    e = element.find(sub)
    if e != None: e.text = text
...
people_data = [Person(*item.split(' ')) for item in split_str(config['people'].get('person'))]
people_index = 0
...
def post_painting(post, body):
    ...
    if has_image and p == None: p = ET.SubElement(post, 'painting')
    if p != None: default_elements(p, painting_default)
    if has_image:
        post.find('body').text = ''
...
def topic_element(data: Topic):
    ...
    p = topic.find('people')
    for i, body in enumerate(data.posts): p.append(person_element(body, data.reply_count[i % len(data.reply_count)]))
    return topic
...
def result_element():
    ...
    for data in topic_data: topics.append(topic_element(data))
    return result
...
xml = '<?xml version="1.0" encoding="UTF-8"?>\n' + ET.tostring(tree.getroot(), encoding = 'utf-8', method = 'xml', short_empty_elements = False).decode('ascii')

if args.output != None:
    with open(args.output, 'w') as f: f.write(xml)
